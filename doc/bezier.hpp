...
ImVec2 ImV2Add(ImVec2 a, ImVec2 b)
{
  return ImVec2(a.x+b.x, a.y+b.y);
}

ImVec2 ImV2Sub(ImVec2 a, ImVec2 b)
{
  return ImVec2(a.x-b.x, a.y-b.y);
}

ImVec2 ImV2Mul(ImVec2 a, ImVec2 b)
{
  return ImVec2(a.x*b.x, a.y*b.y);
}
...
namespace ImGui 
{
  ...
  int Bezier( const char *label, float P[4] ) {
    ...
    ImRect bb(Window->DC.CursorPos, ImV2Add(Window->DC.CursorPos, Canvas));
    ItemSize(bb);
    if (!ItemAdd(bb, NULL))
        return changed;

    const ImGuiID id = Window->GetID(label);
    //hovered |= 0 != IsHovered(ImRect(bb.Min, ImV2Add(bb.Min,ImVec2(avail,dim))), id);
hovered = 1;
    ...
    // control points: 2 lines and 2 circles
    {
      ...
      for(int i = 0; i < 2; ++i)
      {
        ImVec2 pos = ImV2Add(ImV2Mul(ImVec2( P[i*2+0], 1 - P[i*2+1] ), ImV2Sub(bb.Max, bb.Min)), bb.Min);
        SetCursorScreenPos(ImV2Sub(pos, ImVec2(GRAB_RADIUS, GRAB_RADIUS)));
      ...
      if(hovered||changed) DrawList->PushClipRectFullScreen();
        ...
        ImVec2 p1 = ImV2Add(ImV2Mul(ImVec2( P[0], 1 - P[1] ), ImV2Sub(bb.Max, bb.Min)), bb.Min);
        ImVec2 p2 = ImV2Add(ImV2Mul( ImVec2( P[2], 1 - P[3] ), ImV2Sub(bb.Max, bb.Min)), bb.Min);
